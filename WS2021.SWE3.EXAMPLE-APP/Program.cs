...
namespace WS2021.SWE3.EXAMPLE_APP
{
    public class Program
    {
        ...
        public static void Main(string[] args)
        {
            ...
            var queryGroup1 = dbSetup.BookRepository.CreateQuery().Equals((book) => book.Name, "asds").Or().LesserThan((book) => book.Price, 2);
            var queryGroup2 = dbSetup.BookRepository.CreateQuery().Like((book) => book.IBan, "*ass").Or().LesserThan((book) => book.Price, 2).And().In((book) => book.Id, new() { "1", "2", "3" });
            var queryGroup3 = dbSetup.BookRepository.CreateQuery().In((book) => book.Id, new() { "2", "3" }).Or().LesserThan((book) => book.Price, 40);
